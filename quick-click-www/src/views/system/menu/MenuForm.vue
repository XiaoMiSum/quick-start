    ...
    >
      ...
      <el-form-item label="菜单类型" prop="type">
        <el-radio-group v-model="formData.type">
          <el-radio-button v-for="dict in ENUMS.MENU_TYPE_ENUMS" :key="dict.key" :label="dict.key">
            {{ dict.label }}
          </el-radio-button>
        </el-radio-group>
      </el-form-item>
      ...
      <el-form-item label="菜单状态" prop="status">
        <el-radio-group v-model="formData.status">
          <el-radio
            v-for="dict in ENUMS.COMMON_STATUS_ENUMS"
            :key="dict.key"
            :label="dict.key"
            border
          >
...
const formData = ref({
  ...
  component: '',
  componentName: '',
  status: ENUMS.COMMON_STATUS_ENUM.ENABLE,
  visible: true,
  keepAlive: true,
  alwaysShow: true,
  pageType: 2
...
const submitForm = async () => {
  ...
  try {
    if (
      formData.value.type === ENUMS.MENU_TYPE_ENUM.DIR ||
      formData.value.type === ENUMS.MENU_TYPE_ENUM.MENU
    ) {
